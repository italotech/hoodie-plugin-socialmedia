...
module.exports = function (hoodie, pluginDb, userDb) {
  ...
  var _updateComment = function (task, db, cb) {
    ...
    var commentId = task.commentObject.id;
    ...
    _findPost(task, task.postObject.owner.db, function (err, _task) {
      ...
      doc.comments.map(function (comment, i) {
        ...
        doc.comments[i] = task.commentObject;
        doc.comments[i].id = comment.id;

        return _updatePost(task, task.postObject.owner.db, cb);
      });
    });
  };
  ...
  var _sharePost = function (task, db, cb) {
    ...
    _findPost(task, task.postObject.owner.db, function (err, _task) {
      var doc = _task.postObject;
      doc.sourceId = _task.postObject.id;
      delete doc._rev;
      _post(task, db, cb);
    });
  };
  ...
  var _createNotification = function (task, db, notificationType, cb) {
    ...
    var _db = new ExtendedDatabaseAPI(hoodie, hoodie.database('user/' + task.userId));
    _db.add('notification', { id: (Date.now()).toString(36), notificationType: notificationType, from: db.split('/').pop(), to: task.userId }, cb);
  };
...
//        async.apply(_setAttrs, task, 'userId'),
  ...
  SocialMedia.updateComment = function (db, task) {
    ...
    async.series([
        ...
        async.apply(_setAttrs, task.postObject, 'id'),
        async.apply(_setAttrs, task, 'commentObject'),
        async.apply(_setAttrs, task.commentObject, 'id'),
  ...
  SocialMedia.requestFriend = function (db, task) {
