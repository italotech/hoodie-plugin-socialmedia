...
var schemas = require('requireindex')('lib/schema');
var tv4 = require('tv4');
console.log(schemas);
module.exports = function (hoodie, dbPluginProfile) {
  ...
  var _validate = function (task, db, schema, cb) {
    if (!schemas && !schemas.tasks && !schemas.tasks[schema])
      return cb('schema task: ' + schema + ' not found');
    debugger;
    tv4.validate(task, schemas.tasks[schema], function (isValid, validationError) {
      cb(validationError);
    })
  }
  ...
  var _getProfilesAsObject = function (task, db, cb) {
    ...
    if (!task.socialmedia.feed)
      return cb();
    var ids =  task.socialmedia.feed.map(function (v, k, a) {
  ...
  SocialMedia.lookup = function (db, task, cb) {
    ...
    async.series([
        async.apply(_validate, task, db, 'lookup'),
  ...
  SocialMedia.follow = function (db, task) {
    ...
    async.series([
        async.apply(_validate, task, db, 'follow'),
        async.apply(_lookup, task, db),
  ...
  SocialMedia.unfollow = function (db, task) {
    ...
    async.series([
        async.apply(_validate, task, db, 'unfollow'),
        async.apply(_lookup, task, db),
  ...
  SocialMedia.dualFollow = function (db, task) {
    ...
    async.series([
        async.apply(_validate, task, db, 'dualFollow'),
  ...
  SocialMedia.post = function (db, task) {
    ...
    async.series([
        async.apply(_validate, task, db, 'post'),
  ...
  SocialMedia.getPost = function (db, task) {
    ...
    async.series([
        async.apply(_validate, task, db, 'getPost'),
  ...
  SocialMedia.updatePost = function (db, task) {
    ...
    async.series([
        async.apply(_validate, task, db, 'updatePost'),
  ...
  SocialMedia.deletePost = function (db, task) {
    ...
    async.series([
        async.apply(_validate, task, db, 'deletePost'),
  ...
  SocialMedia.feed = function (db, task) {
    ...
    async.series([
        async.apply(_validate, task, db, 'feed'),
  ...
  SocialMedia.comment = function (db, task) {
    ...
    async.series([
        async.apply(_validate, task, db, 'comment'),
        async.apply(_findPost, task, db),
  ...
  SocialMedia.count = function (db, task) {
    ...
    async.series([
        async.apply(_validate, task, db, 'count'),
        async.apply(_findPost, task, db),
  ...
  SocialMedia.uncount = function (db, task) {
    ...
    async.series([
        async.apply(_validate, task, db, 'uncount'),
        async.apply(_findPost, task, db),
  ...
  SocialMedia.updateComment = function (db, task) {
    ...
    async.series([
        async.apply(_validate, task, db, 'updateComment'),
  ...
  SocialMedia.deleteComment = function (db, task) {
    ...
    async.series([
        async.apply(_validate, task, db, 'deleteComment'),
  ...
  SocialMedia.sharePost = function (db, task) {
    ...
    async.series([
        async.apply(_validate, task, db, 'sharePost'),
